    ...
    Runnable r = new Runnable() {
	  ...
	  if(evt!=null) ssh.addEventHandler(evt);
          ...
          if (postConnection()) {
        	...
        	//To stop error message to be printed when user cancel login
        	if (!sshe.getMessage().equals("Canceled by user.")){	
        		sshe.printStackTrace();
        		log.error("Thrown (IOException): "+sshe);
        		showExceptionMessage("Connection Failed", sshe.getMessage() + "\nPlease check that the hostname and port are valid. \nIf you have specified a username, please ensure that you have an account with that username on the remote system.");
        	}
          ssh = null;
	  if(sshe.getMessage().indexOf("Canceled by user")<0) {
          /* jsiwek: user should not be notified of intentionally cancelled
              connection.  Further, if this is the result of exiting
              an SshTerm applet, then the message dialog box is orphaned
              and leaves no way exit or refocus the browser on some operating
   ...
   */
	...
	try {
	    ...
	    java.util.List display = new java.util.ArrayList();
            ...
            /* jsiwek: I disabled using a popup to display the userauth banner
                       in favor of rendering it to the terminal emulator.
                       Also, there was the potential for 'gssapi-keyex' or
                       'public-key' authentication to skip the banner display
                       routine, so it's been moved up above them.
            */
