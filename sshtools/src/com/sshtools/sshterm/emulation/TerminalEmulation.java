...
public class TerminalEmulation
  ...
  //Cerlane adds Mac Command Key (Meta)
  private String[] FunctionKeyMeta;
  ...
  public void setKeyCodes(Properties codes) {
    ...
    for (i = 1; i < 20; i++) {
      ...
      //Cerlane adds Mac Command key (meta)
      res = codes.getProperty("MF" + i);
      if (res != null) {
        FunctionKeyMeta[i] = unEscape(res);
      }
    }
    for (i = 0; i < 4; i++) {
  ...
  public void keyPressed(int keyCode, char keyChar, int modifiers) {
    ...
    //Cerlane adds Mac Command key (Meta)
    boolean meta = (modifiers & VDUInput.KEY_META) != 0;
  ...
  public void keyTyped(int keyCode, char keyChar, int modifiers) {
    ...
    boolean meta = (modifiers & VDUInput.KEY_META) != 0;
    // fix problem with Linux Java 6 -- really nasty
    if(meta && keyChar>='a' && keyChar<='z') keyChar=(char)(keyChar-96);
    if(meta && shift && keyChar>='A' && keyChar<='Z') keyChar=(char)(keyChar-64);
