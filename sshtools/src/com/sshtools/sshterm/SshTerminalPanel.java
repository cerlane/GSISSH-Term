...
/*, MouseWheelListener*/{
  ...
  //Added by Cerlane
  public final static String PREF_MYPROXY_VONAME=
      "sshterm.myproxy.voname";  
  public final static String PREF_MYPROXY_CACERT_URL=
	  "sshterm.myproxy.cacert.url";
  public final static String PREF_VOMS_CONFIG_URL=
	  "sshterm.voms.config.url";
  public final static String PREF_VOMS_LOCATION=
	  "sshterm.voms.location";
  public final static String PREF_VOMS_PROXYTYPE =
	  "sshterm.voms.proxyType";
  public final static String PREF_GO_USERNAME=
	  "sshterm.go.username";
  //End added by Cerlane
  ...
  public final static String PREF_PKCS12_DEFAULT_FILE =
  ...
  //Added by Cerlane
  public final static String PREF_TERMINAL_TERM = 
	  "sshterm.terminal.term";
  public final static String PREF_VOMS_SUPPORT =
      "sshterm.voms.support";
  //End added by Cerlane
  ...
  // Connected status
  //jsiwek: changed this to be inherited from SshToolsApplicationClientPanel
  ...
  public void actionPerformed(ActionEvent event) {
    ...
    //  MyProxy Tool
    if ( (myproxytoolAction != null)
        && event.getActionCommand().equals(myproxytoolAction
                                           .getActionCommand())) {
      if (myproxytoolFrame == null) {
    	  myproxytoolFrame = new com.sshtools.common.myproxytool.Main();
    	  myproxytoolFrame.setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
    	  myproxytoolFrame.pack();
        UIUtil.positionComponent(SwingConstants.CENTER, myproxytoolFrame);
      }

      if (!myproxytoolFrame.isVisible()) {
        setFullScreenMode(false);
      }

      myproxytoolFrame.setVisible(!myproxytoolFrame.isVisible());
    }
    
    //  Globus Online Tool
    if ( (gotoolAction != null)
        && event.getActionCommand().equals(gotoolAction
                                           .getActionCommand())) {
      if (gotoolFrame == null) {
    	  gotoolFrame = new com.sshtools.common.globusonlinetool.Main();
    	  if (gotoolFrame.isToContinue()){
	    	  gotoolFrame.setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
	    	  gotoolFrame.pack();
	    	  UIUtil.positionComponent(SwingConstants.CENTER, gotoolFrame);
    	  }
      }
      else if (!GOHelper.isAuthenticated()){
    	  gotoolFrame = new com.sshtools.common.globusonlinetool.Main();
    	  if (gotoolFrame.isToContinue()){
	    	  gotoolFrame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
	    	  gotoolFrame.pack();
	    	  UIUtil.positionComponent(SwingConstants.CENTER, gotoolFrame);
    	  }
      }
      
      if (!gotoolFrame.isVisible()) {
        setFullScreenMode(false);
      }
      if (gotoolFrame.isToContinue()){
    	  gotoolFrame.setVisible(!gotoolFrame.isVisible());
      }
    }
    
    //  VO Management Tool
    if ( (votoolAction != null)
        && event.getActionCommand().equals(votoolAction
                                           .getActionCommand())) {
      if (votoolFrame == null) {
    	  votoolFrame = new com.sshtools.common.vomanagementtool.Main();
    	  votoolFrame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
    	  votoolFrame.pack();
    	  UIUtil.positionComponent(SwingConstants.CENTER, votoolFrame);
    	  votoolFrame.addWindowListener(new java.awt.event.WindowAdapter() {
      ...
      if (!votoolFrame.isVisible()) {
        setFullScreenMode(false);
      }

      votoolFrame.setVisible(!votoolFrame.isVisible());
    }
  ...
  public void connect(final SshToolsConnectionProfile profile,
	...
	Exception e = transport.getState().getLastError();
        ...
        /*
        // jsiwek no processing needed for now, but this would be where to
        //  catch the additional transport timeout that I've added
        if (e!=null && e instanceof java.io.EOFException && 
            e.getMessage().indexOf("Connection timed out")>=0) {
        }
        */
  ...
  private void setTerminalProperties(SshToolsConnectionProfile profile) {
    ...
    terminal.setColorPrinting(profile.getApplicationPropertyBoolean(
        PROFILE_PROPERTY_COLOR_PRINTING, true));
  ...
  private void initActions() {
    ...
    // register Help menu items which don't depend upon application knowledge/state
    //  those that do will be registered in the SshToolsApplicationFrame class
    registerAction(faqAction = new FAQAction());
    registerAction(beginnerAction = new BeginnerAction());
    registerAction(advancedAction = new AdvancedAction());
    ...
    //Myproxy tool (create, remove and retrieve info)
    try {
        
        myproxytoolAction = new MyProxyToolAction();
        registerAction(myproxytoolAction);
      }
      catch (AccessControlException ace) {
        log.warn("MyProxy Tool action is not available");
      }
      //Globus Online tool (transfer files via GridFTP)
      try {

    	  gotoolAction = new GlobusOnlineToolAction();
    	  registerAction(gotoolAction);
      }
      catch (AccessControlException ace) {
    	  log.warn("Globus Online Tool action is not available");
      }

      //VO Management tool (management voms setup)
      try {

    	  votoolAction = new VOManagementToolAction();
    	  registerAction(votoolAction);
      }
      catch (AccessControlException ace) {
  ...
  public void setAvailableActions() {
    ...
    if(faqAction != null) {
      faqAction.setEnabled(true);
    }

    if(beginnerAction != null) {
      beginnerAction.setEnabled(true);
    }

    if(advancedAction != null) {
      advancedAction.setEnabled(true);
    }
