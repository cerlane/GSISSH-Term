 ...
 *  
 *  [2011-04-06] Cerlane: added variable Keyboard_Modifier and Function getKeyboardModifier to predefine keyboard modifier (shortcuts)
 *
 */
...
public abstract class SshToolsApplicationPanel
  ...
  /**This is added to make the keyboard modifier for different OS easily configurable */
  
  protected static int Keyboard_Modifier = getKeyboardModifier();
    ...
    for (Iterator i = menus.iterator(); i.hasNext(); ) {
      ...
      if (x != null) {
        ...
        //log.error("Can't find menu " + m.name);
          log.warn("Menu '"+m.name + "' has no associated registered Actions");
      }
    }
    menuBar.validate();
    menuBar.repaint();
  ...
  *
  ...
  public static void showErrorMessage(Component parent, String mesg,
    ...
    while (true) {
      ...
      /*
       * Patch 2 contributed by Jon Siwek
       * Error popups sometimes extended off the visible display,
       * this patch will cause them to word wrap at 130 characters.
       */
      appendException(exception, 0, buf, details);
      JOptionPane wrappedPane = getNarrowOptionPane(130);
      wrappedPane.setMessage(buf.toString());
      wrappedPane.setMessageType(JOptionPane.ERROR_MESSAGE);
      wrappedPane.setOptionType(JOptionPane.OK_CANCEL_OPTION);
      wrappedPane.setOptions(opts);
      wrappedPane.setInitialValue(opts[1]);
      javax.swing.JDialog dialog = wrappedPane.createDialog(parent, title);
      dialog.setVisible(true); //dialog.show();
      Object selectedValue = wrappedPane.getValue();
      int opt = 1;
      for (int counter = 0, maxCounter = opts.length; counter < maxCounter; counter++)
      if (opts[counter].equals(selectedValue)) opt = counter;
      /*
       * End Patch 2
       */

      /*
       * Orignal unmodified srce below
       *
   ...
   * @return
   */    
  public static int getKeyboardModifier(){
